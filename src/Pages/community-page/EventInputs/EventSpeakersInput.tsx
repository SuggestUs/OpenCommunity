...
export default function EventSpeakersCom() {
  ...
  const [arrayForSpeakers, setArrayForSpeakers] =
    useState<typeForSpeakersArray>([
      {
        speaker_file: null,
        speaker_url: "",
        speaker_name: "",
  ...
  return (
    <>
      <div className="flex flex-col h-screen w-full justify-center">
        <div
          className="flex md:flex-row flex-col h-auto  w-full flex-wrap justify-normal:md justify-center"
          id="containerForSpeakers"
        >
          {arrayForSpeakers.map((_item, index) => {
            return (
              <div
                className="flex flex-col my-4 mx-4 justify-center border p-5 hover:border-gray-600"
                key={index}
              >
                <div className="flex w-full justify-end h-10">
                  {index !== 0 && (
                    <Button
                      variant="outlined"
                      className=""
                      onClick={() => deleteProfileCard(index)}
                    >
                      D
                    </Button>
                  )}
                ...
                <div className=" flex flex-col items-center mx-10 justify-center mt-4">
                  ...
                  >
                    <input
                      ...
                      style={{ display: "none" }}
                      onChange={(event) =>
                        changeProfileImageForSpeakes(event, index)
                      }
                    />
                  </Avatar>
                </div>
